...
describe('[COMPILER]', () => {
    describe('assert.token', () => {
        it.each([
            ...
            expect(() => assert.token([], json, path)).toThrow(error);
        });
        ...
        it('rejects JSON parse errors', () => {
            ...
            expect(() => assert.token(errors, { component: 'button', vars: {} }, path)).toThrow('Invalid JSON');
        });
    });
    describe('assert.variable', () => {
        it.each([
            ...
            expect(() => assert.variable('background', value, path)).toThrow(error);
        });
        it('accepts valid variables', () => {
            expect(() => assert.variable('background', {
                name: 'Background',
                allowed: ['f'],
                exclude: ['p'],
                values: {
    ...
    describe('assert.cssVariable', () => {
        it.each([
            ...
            expect(() => assert.cssVariable(value)).not.toThrow();
        });
        it.each([
            'button-background',
            '-button-background',
            ...
            expect(() => assert.cssVariable(value)).toThrow('is not a CSS custom property');
        });
    });
    describe('assert.hasCssPath', () => {
        it('accepts groups with cssPath', () => {
            ...
            expect(() => assert.hasCssPath(group)).not.toThrow();
        });
        it('rejects undefined groups', () => {
            expect(() => assert.hasCssPath(undefined)).toThrow('has no cssPath');
        });
        it('rejects groups without cssPath', () => {
            ...
            expect(() => assert.hasCssPath(group)).toThrow('has no cssPath');
        });
    });
    describe('assert.groupsHaveCssPath', () => {
        it('accepts groups with cssPath', () => {
            ...
            expect(() => assert.groupsHaveCssPath(groups)).not.toThrow();
        });
        it('rejects a group without cssPath', () => {
            ...
            expect(() => assert.groupsHaveCssPath(groups)).toThrow('Token group "/tokens/input" has no cssPath');
        });
        it('returns without error for an empty group list', () => {
            expect(() => assert.groupsHaveCssPath([])).not.toThrow();
        });
    });
    describe('assert.groupsHaveCssData', () => {
        it('accepts groups with cssData', () => {
            ...
            expect(() => assert.groupsHaveCssData(groups)).not.toThrow();
        });
        it('rejects a group without cssData', () => {
            ...
            expect(() => assert.groupsHaveCssData(groups)).toThrow('A token in tokenGroups "/tokens/input" has no cssData');
        });
        it('returns without error for an empty group list', () => {
            expect(() => assert.groupsHaveCssData([])).not.toThrow();
        });
    });
});
