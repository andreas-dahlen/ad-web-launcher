...
describe('[COMPILER]', () => {
    describe('parseToken.identifier', () => {
        ...
        it('throws when nothing remains after parsing', () => {
            expect(() => parseToken.identifier('!!!')).toThrow('Identifier was empty after parsing');
        });
    });
    ...
    describe('parseToken.variable', () => {
        it('uses the key as the default variable name', () => {
            const result = parseToken.variable(createRawVariable(), 'background', [], createTestCollector());
        ...
        it('uses the explicit variable name when provided', () => {
            const result = parseToken.variable(createRawVariable({ name: 'Background' }), 'bg', [], createTestCollector());
            expect(result.variable.name).toBe('background');
            expect(result.variable.key).toBe('bg');
        });
        it('removes whitespace from the explicit variable name', () => {
            const result = parseToken.variable(createRawVariable({ name: 'background Color' }), 'bg', [], createTestCollector());
            expect(result.variable.name)
                .toBe('backgroundColor');
        });
        it('resolves effective allowed prefixes', () => {
            const result = parseToken.variable(createRawVariable({ allowed: ['o'], exclude: ['p'] }), 'bg', ['f'], createTestCollector());
        ...
        it('parses variable values', () => {
            const result = parseToken.variable(createRawVariable({
                values: { f: 'black', p: 'red' }
            }), 'bg', [], createTestCollector());
