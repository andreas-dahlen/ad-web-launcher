...
function createCssModule(relativePath) {
...
describe("[COMPILER]", () => {
    describe("findCssModulePath", () => {
        it("finds a matching css module", () => {
            ...
            expect(findCssModulePath("/tokens/button", tmpRoot)).toBe(file);
        });
        ...
        it("searches nested directories", () => {
            const file = createCssModule("components/buttons/Button.module.css");
            expect(findCssModulePath("/tokens/button", tmpRoot)).toBe(file);
        });
        it("returns undefined when no matching css module exists", () => {
            createCssModule("Surface.module.css");
            expect(findCssModulePath("/tokens/button", tmpRoot)).toBeUndefined();
        });
    });
});
