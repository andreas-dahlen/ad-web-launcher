...
import { createCompilerToken, } from '../compiler.factory';
vi.mock('@styleTokens/compiler/discovery/createModuleMap', () => ({
    createModuleMap: vi.fn(),
}));
vi.mock('@styleTokens/compiler/processing/processToken', () => ({
    processToken: vi.fn(),
}));
describe('[COMPILER]', () => {
    describe('compileTokenGroups', () => {
        it('returns no groups or issues for an empty token list', () => {
            vi.mocked(createModuleMap).mockReturnValue(new Map());
        ...
        it('processes every token path', () => {
            ...
            expect(processToken).toHaveBeenNthCalledWith(1, paths[0]);
            expect(processToken).toHaveBeenNthCalledWith(2, paths[1]);
        });
        ...
        it('preserves token order within a group', () => {
            ...
            const tokens = paths.map((tokenPath, index) => createCompilerToken({
                tokenPath,
                infix: `state-${index}`,
            }));
            vi.mocked(createModuleMap).mockReturnValue(new Map());
