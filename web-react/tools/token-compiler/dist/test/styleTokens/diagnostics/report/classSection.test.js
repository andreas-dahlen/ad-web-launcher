...
function createMissingClass(overrides = {}) {
    return {
        infix: 'button',
...
describe('[DIAGNOSTICS]', () => {
    describe('classSection', () => {
        ...
        it('identifies the missing class in the entry title', () => {
            ...
            expect(result?.entries[0].title).toContain('Expected:');
        });
        it('includes the token file path', () => {
            ...
            expect(result?.entries[0].lines).toContain('File: /tokens/button/default.jsonc');
        });
        it('includes the number of available selectors', () => {
            ...
            expect(lines.some(line => line.includes('Available selectors (3)'))).toBe(true);
        });
        it('includes all available selectors', () => {
            ...
            const lines = result?.entries[0].lines ?? [];
            const selectorLine = lines.find(line => line.includes('button') &&
                line.includes('card') &&
                line.includes('input'));
        ...
        it('does not include a file line when the token path is empty', () => {
            ...
            expect(result?.entries[0].lines).not.toContain(expect.stringContaining('File:'));
        });
        ...
        it('reports the number of generated entries in the section title', () => {
            ...
            expect(result?.title).toContain('Missing Css Classes');
