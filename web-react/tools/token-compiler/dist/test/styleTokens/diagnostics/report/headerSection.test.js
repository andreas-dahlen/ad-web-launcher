...
describe('[DIAGNOSTICS]', () => {
    describe('headerSection', () => {
        ...
        it('uses update mode for zero processed modules', () => {
            ...
            expect(result.entries[0].title).not.toContain('Initialization complete!');
        });
        it('uses initialization mode only when more than one module is processed', () => {
            const result = headerSection(2);
            expect(result.entries[0].title).toContain('Initialization complete!');
            const single = headerSection(1);
            expect(single.entries[0].title).not.toContain('Initialization complete!');
        });
    });
});
