...
describe('[DIAGNOSTICS]', () => {
    describe('fileSection', () => {
        ...
        it('creates an entry for each missing file', () => {
            ...
            expect(titles[0]).toContain('/components/Button.module.css');
            expect(titles[1]).toContain('/components/Card.module.css');
        });
        it('adds the module CSS suffix to the file path', () => {
            ...
            expect(entry?.title).toContain('/components/Button.module.css');
        });
        it('identifies entries as files', () => {
            const result = fileSection([
                '/components/Button',
            ]);
            expect(result?.entries[0].title).toContain('File:');
        });
        it('ignores empty file paths', () => {
            ...
            expect(result?.entries).toHaveLength(1);
            expect(result?.entries[0].title).toContain('/components/Button.module.css');
        });
