...
describe('[DIAGNOSTICS]', () => {
    describe('tokenSection', () => {
        ...
        it('reports written files in a written entry', () => {
            ...
            expect(entry?.lines?.some(line => line.includes('tokens/button.token.ts'))).toBe(true);
            expect(entry?.lines?.some(line => line.includes('tokens/card.token.ts'))).toBe(true);
        });
        ...
        it('returns undefined when there are no files', () => {
            expect(tokenSection(createFileStatus())).toBeUndefined();
        });
    });
});
