...
function createUnusableSelector(overrides = {}) {
    return {
...
describe('[DIAGNOSTICS]', () => {
    describe('selectorSection', () => {
        it('creates a section for unusable selectors', () => {
            ...
            expect(result?.title).toContain('Unusable Preset Selectors');
        ...
        it('creates an entry for each affected file', () => {
            ...
            expect(titles[0]).toContain('/components/Button/Button.module.css');
            expect(titles[1]).toContain('/components/Card/Card.module.css');
        });
        it('includes the unusable selector count', () => {
            ...
            expect(lines.some(line => line.includes('Selectors') &&
        ...
        it('ignores empty selector groups while keeping valid groups', () => {
            ...
            expect(result?.entries).toHaveLength(1);
            expect(result?.entries[0].title).toContain('/components/Button/Button.module.css');
        });
