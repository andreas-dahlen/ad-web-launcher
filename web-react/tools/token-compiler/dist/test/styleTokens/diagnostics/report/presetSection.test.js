...
function createFileStatus(overrides = {}) {
    return {
        written: [],
        skipped: [],
        ...overrides,
...
describe('[DIAGNOSTICS]', () => {
    describe('presetSection', () => {
        ...
        it('reports formatted written file paths', () => {
            ...
            const entry = result?.entries.find(entry => entry.title.includes('written'));
            expect(entry).toBeDefined();
            expect(entry?.lines).toHaveLength(2);
            expect(entry?.lines?.some(line => line.includes('presets/button.preset.ts'))).toBe(true);
            expect(entry?.lines?.some(line => line.includes('presets/card.preset.ts'))).toBe(true);
        });
        it('reports formatted skipped file paths', () => {
            ...
            const entry = result?.entries.find(entry => entry.title.includes('skipped'));
            expect(entry).toBeDefined();
            expect(entry?.lines).toHaveLength(2);
            expect(entry?.lines?.some(line => line.includes('presets/button.preset.ts'))).toBe(true);
            expect(entry?.lines?.some(line => line.includes('presets/card.preset.ts'))).toBe(true);
        ...
        it('creates separate entries for written and skipped files', () => {
            ...
            expect(result?.entries.some(entry => entry.title.includes('written'))).toBe(true);
            expect(result?.entries.some(entry => entry.title.includes('skipped'))).toBe(true);
        });
        ...
        it('does not create a written entry when no files were written', () => {
            ...
            expect(result?.entries.some(entry => entry.title.includes('written'))).toBe(false);
        });
        it('does not create a skipped entry when no files were skipped', () => {
            ...
            expect(result?.entries.some(entry => entry.title.includes('skipped'))).toBe(false);
        });
        it('returns undefined when there are no files', () => {
            expect(presetSection(createFileStatus())).toBeUndefined();
        });
    });
});
