...
function createVariableMismatch(overrides = {}) {
    return {
        name: 'button',
        infix: 'button',
        missing: [],
...
describe('[DIAGNOSTICS]', () => {
    describe('variableSection', () => {
        ...
        it('uses the infix as the component name when name and infix match', () => {
            ...
            expect(result?.entries[0].title).toContain('Component: button');
        });
        it('combines name and infix when they differ', () => {
            ...
            expect(result?.entries[0].title).toContain('Component: surface-button');
        });
        it('reports unused variables', () => {
            ...
            expect(lines.some(line => line.includes('Unused in:') &&
                line.includes('CSS') &&
                line.includes('(2)'))).toBe(true);
            expect(lines.some(line => line.includes('--s-button-color'))).toBe(true);
            expect(lines.some(line => line.includes('--s-button-radius'))).toBe(true);
        });
        it('reports missing variables', () => {
            ...
            const lines = result?.entries[0].lines ?? [];
            expect(lines.some(line => line.includes('Missing in:') &&
                line.includes('JSON') &&
                line.includes('(2)'))).toBe(true);
            expect(lines.some(line => line.includes('--final-button-color'))).toBe(true);
            expect(lines.some(line => line.includes('--final-button-radius'))).toBe(true);
        });
        it('reports unused and missing variables together', () => {
            ...
            const lines = result?.entries[0].lines ?? [];
            expect(lines.some(line => line.includes('Unused in:'))).toBe(true);
            expect(lines.some(line => line.includes('Missing in:'))).toBe(true);
            expect(lines.some(line => line.includes('--s-button-color'))).toBe(true);
            expect(lines.some(line => line.includes('--final-button-radius'))).toBe(true);
        });
