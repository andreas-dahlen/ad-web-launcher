...
vi.mock('@styleTokens/utils/string', () => ({
    ...
    paint: String,
    formatLogPath: (value) => value,
}));
function createIssueGroup(overrides = {}) {
    return {
...
describe('[DIAGNOSTICS]', () => {
    describe('issuesSection', () => {
        ...
        it('uses the context name in context lines', () => {
            ...
            expect(result?.entries[0].lines?.[0]).toContain('button');
            expect(result?.entries[0].lines?.[0]).toContain('(0)');
        });
        it('uses general when a context has no name', () => {
            ...
            expect(result?.entries[0].lines?.[0]).toContain('general');
        });
        it('reports the number of issues in each context', () => {
            ...
            expect(result?.entries[0].lines?.[0]).toContain('(2)');
        });
        it('includes issue values, reasons, and file paths', () => {
            ...
            expect(lines.some(line => line.includes('old-value'))).toBe(true);
            expect(lines.some(line => line.includes('Invalid value'))).toBe(true);
            expect(lines.some(line => line.includes('/tokens/button/default.jsonc'))).toBe(true);
        });
        it('includes the replacement value when an issue has one', () => {
            ...
            const issueLine = result?.entries[0].lines?.find(line => line.includes('old-value') &&
        ...
        it('does not include a replacement arrow when no replacement exists', () => {
            ...
            const issueLine = result?.entries[0].lines?.find(line => line.includes('Something went wrong'));
            expect(issueLine).toBeDefined();
        ...
        it('creates entries for multiple contexts', () => {
            ...
            const lines = result?.entries[0].lines ?? [];
            expect(lines.some(line => line.includes('button'))).toBe(true);
            expect(lines.some(line => line.includes('surface'))).toBe(true);
        });
