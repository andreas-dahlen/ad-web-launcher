...
import { writeFiles } from '@styleTokens/emitters/write/writeFiles';
describe('[EMITTER]', () => {
    let tempDir;
    beforeEach(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'style-tokens-write-'));
    });
    afterEach(() => {
        fs.rmSync(tempDir, {
            recursive: true,
    ...
    describe('writeFiles', () => {
        it('writes a new file', () => {
            ...
            expect(fs.readFileSync(filePath, 'utf8')).toBe('export const button = {}');
        });
        it('creates missing parent directories', () => {
            const filePath = path.join(tempDir, 'generated', 'tokens', 'button.ts');
            const result = writeFiles([
                {
                    filePath,
                    content: 'generated',
        ...
        it('updates an existing file when content differs', () => {
            const filePath = path.join(tempDir, 'button.ts');
            fs.writeFileSync(filePath, 'export const button = "old"');
            const result = writeFiles([
                {
                    filePath,
            ...
            expect(fs.readFileSync(filePath, 'utf8')).toBe('export const button = "new"');
        });
        it('processes multiple files independently', () => {
            ...
            fs.writeFileSync(skippedPath, 'same');
            fs.writeFileSync(updatedPath, 'old');
            const result = writeFiles([
                {
                    filePath: newPath,
                    content: 'new',
