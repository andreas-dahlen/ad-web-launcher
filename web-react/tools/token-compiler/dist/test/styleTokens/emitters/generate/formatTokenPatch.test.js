    ...
    describe('formatTokenPatch', () => {
        ...
        it('creates a JSONC patch pointing to the CSS file', () => {
            ...
            expect(result).toEqual({
                ...
                content: '// file://wsl.localhost/Ubuntu/components/Button/Button.module.css',
            });
        });
        it('creates a CSS patch containing all token files', () => {
            ...
            const result = results.find(file => file.filePath === '/components/Button/Button.module.css');
            expect(result).toEqual({
        ...
        it('creates one JSONC patch for each JSONC token file', () => {
            ...
            expect(results.filter(file => file.filePath.endsWith('.jsonc'))).toHaveLength(2);
        });
        ...
        it('does not create a CSS patch for a non-CSS file', () => {
            ...
            expect(results.some(file => file.filePath === '/components/Button/Button.module')).toBe(false);
            expect(results.some(file => file.filePath === '/tokens/button/default.jsonc')).toBe(true);
        });
