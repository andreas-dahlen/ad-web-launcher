...
function getRule(root, selector) {
...
function getDeclarations(root, selector) {
    return getRule(root, selector).nodes?.filter(node => node.type === 'decl') ?? [];
}
describe('[POSTCSS]', () => {
    describe('processModule', () => {
      ...
      `);
            ...
            expect(result.cssPath).toBe('/components/Button/Button.module.css');
