...
import { injectCascade } from '@styleTokens/postCss/inject/injectCascade';
function createRule() {
...
function getLastDeclaration(rule) {
...
describe('[POSTCSS]', () => {
    describe('injectCascade', () => {
        it('appends a final variable using the allowed prefix', () => {
            ...
            expect(declaration.prop).toBe('--final-button-back-ground');
            expect(declaration.value).toBe('var(--f-button-back-ground)');
        });
        it('builds a fallback chain from multiple allowed prefixes', () => {
            ...
            const declaration = getLastDeclaration(rule);
            expect(declaration.prop).toBe('--final-button-back-ground');
            expect(declaration.value).toBe('var(--p-button-back-ground, var(--f-button-back-ground))');
        });
        ...
        it('uses the token infix when building variable names', () => {
            ...
            injectCascade(rule, token, variable);
            const declaration = getLastDeclaration(rule);
            expect(declaration.prop).toBe('--final-surface-border-radius');
            expect(declaration.value).toBe('var(--f-surface-border-radius)');
        });
    });
});
