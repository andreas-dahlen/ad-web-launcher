...
export const assert = {
    ...
    variable(key, def, fullPath) {
        ...
        if (variable.allowed !== undefined && !Array.isArray(variable.allowed)) {
            throw new Error(`❌ Variable "${key}" allowed must be an array in ${fullPath}`);
        }
    ...
    cssVariable(value) {
        if (!CSS_VARIABLE.test(value)) {
            throw new Error(`Invariant violated: "${value}" is not a CSS custom property.`);
        }
    },
    hasCssPath(group) {
        if (!group?.cssPath) {
            throw new Error(`Invariant violated: Token group "${group?.groupPath}" has no cssPath.`);
        }
    },
    ...
    groupsHaveCssData(groups) {
        for (const group of groups) {
            if (!group?.cssData) {
                throw new Error(`Invariant violated: A token in tokenGroups "${group.groupPath}" has no cssData.`);
            }
        }
    }
};
