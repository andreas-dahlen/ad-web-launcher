...
export function issuesSection(groups) {
    const entries = [];
    for (const group of groups) {
        ...
        for (const context of group.contexts) {
            ...
            for (const issue of context.issues) {
                ...
                lines.push(`   [${value}] - ${paint(issue.reason, colors.error)} - ${paint("File", colors.muted)}: ${paint(formatLogPath(issue.path), colors.file)}`);
            }
            if (context.context) {
                lines.push("");
            }
