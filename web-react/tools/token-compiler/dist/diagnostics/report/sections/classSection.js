...
export function classSection(data) {
    const entries = [];
    for (const missingClass of data) {
        const lines = [];
        if (missingClass.tokenPath.length > 0) {
            lines.push(`${paint("File:", colors.muted)} ${paint(formatLogPath(missingClass.tokenPath), colors.file)}`);
        }
        if (missingClass.usableSelectors.length > 0) {
            lines.push(`${paint("Available selectors", colors.subHeading)} (${paint(missingClass.usableSelectors.length, colors.value)}),
