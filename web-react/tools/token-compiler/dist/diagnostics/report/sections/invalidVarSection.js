...
export function invalidVarSection(data) {
    const entries = [];
    for (const variable of data) {
        const lines = [];
        if (variable.invalid.length > 0) {
            lines.push(` ${paint(`🎨 Invalid declarations:`, colors.error)} (${paint(variable.invalid.length, colors.value)})`, ...variable.invalid.map(variable => `    ${paint(variable, colors.symbol)}`));
        }
