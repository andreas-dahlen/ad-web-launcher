...
export function buildData(cache, run) {
    const missingClasses = [];
    const unusableSelectors = [];
    const mismatchedVariables = [];
    const invalidVarDeclarations = [];
    /*---------------------------------------
            NON-Group specific
    -------------------------------------*/
    ...
    const omittedPresetFiles = emitResult?.extractResult.omittedPresetFiles ?? [];
    for (const group of runGroups) {
        ...
        if (selectorResult)
            unusableSelectors.push(selectorResult);
