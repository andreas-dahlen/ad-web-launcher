...
export function walkModule(root, infixes) {
    const expectedRules = new Set(infixes.map(infix => `.${infix}`));
    const variablePrefixes = infixes.map(infix => toCssVarPrefix("final", infix));
    const declarationPrefixes = prefixPriority
        .flatMap(prefix => infixes.map(infix => `--${prefix}-${infix}`));
    const rules = new Map();
    const foundSelectors = new Set();
    const usableSelectors = new Set();
    const foundFinalVariables = new Set();
    const declaredVariables = new Set();
    const presetResetData = new Map();
    root.walkRules(rule => {
    ...
    root.walkDecls(decl => {
        if (declarationPrefixes.some(prefix => decl.prop.startsWith(prefix))) {
        ...
        for (const match of decl.value.matchAll(/var\((--[\w-]+)\s*(?:,[^)]+)?\)/g)) {
            const cssVar = match[1];
            if (variablePrefixes.some(prefix => cssVar.startsWith(prefix))) {
