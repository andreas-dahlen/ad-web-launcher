...
import loadToken, { type LoadedToken } from './loadToken.ts';

export default function loadTokens(path: string): LoadedToken[] {
  const files = findTokenPaths(path);

  return files.map(loadToken);
}
// export default function loadTokens(tokensDir: string): LoadedToken[] {
